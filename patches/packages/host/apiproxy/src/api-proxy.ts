...
function sessionBlank(session: Session): boolean {
  // 群聊插件创建的会话带有 qunliao/setup 事件（建群快照），视为已开始；
  // 否则会话会被列表当作空白会话隐藏，且不显示改名/复制/归档等操作菜单。
  return !session.events.some(event =>
    event.type === 'turn/start' || (event.type as string) === 'qunliao/setup')
}
...
function applySessionListMetadata(state: SessionListMetadata, event: SessionEvent): SessionListMetadata {
  const blank = state.blank && event.type !== 'turn/start' && (event.type as string) !== 'qunliao/setup'
