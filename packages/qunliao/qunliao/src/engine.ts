...
import type {
  QunliaoErrorData, QunliaoMember, QunliaoMessageData, QunliaoMessageStartData,
...
/** 某成员的一次发言（供测试注入的假模型）。 */
export type QunliaoSpeaker = (
  ctx: Context,
  session: Session,
  member: QunliaoMember,
  round: number,
...
export function parseMentions(
  text: string,
  members: readonly QunliaoMember[],
): QunliaoMember[] {
  const seen = new Set<string>()
  const result: QunliaoMember[] = []
...
export function validateCreateInput(
  input: QunliaoCreateInput,
  config: QunliaoEngineConfig,
): string | null {
  const name = input.name.trim()
  if (name.length === 0) return '群名不能为空'
  const members = input.members
  if (members.length < config.minMembers) return `至少需要 ${config.minMembers} 位成员`
  if (members.length > config.maxMembers) return `最多支持 ${config.maxMembers} 位成员`
  const names = new Set<string>()
  for (const member of members) {
    const memberName = member.name.trim()
    if (memberName.length === 0) return '成员名字不能为空'
    if (names.has(memberName)) return `成员名字重复：${memberName}`
    names.add(memberName)
    if (member.identity.trim().length === 0) return `成员「${memberName}」缺少身份设定`
    if (member.provider.trim().length === 0) return `成员「${memberName}」缺少模型提供方`
    if (member.model.trim().length === 0) return `成员「${memberName}」缺少模型`
  }
  return null
}

/** 从事件流还原某会话的群聊记录（用户发言 + 已完成成员发言）。 */
export function buildTranscript(session: Session): QunliaoTranscriptEntry[] {
  const result: QunliaoTranscriptEntry[] = []
  for (const event of session.events) {
    if (event.type === 'user/message') {
      const text = textOfContent(event.data.content)
      if (text.length > 0) result.push({ kind: 'user', speakerName: '用户', text })
...
export class QunliaoEngine {
  private readonly speak: QunliaoSpeaker

  constructor(
    private readonly ctx: Context,
    private readonly config: QunliaoEngineConfig,
    speaker?: QunliaoSpeaker,
  ) {
    this.speak = speaker ?? this.defaultSpeaker
  }

  /** 运行期状态缓存：同一会话共享同一状态实例，保证打断与开关读写一致。 */
  private readonly states = new Map<Session['id'], QunliaoGroupState>()
  ...
  hydrate(session: Session): QunliaoGroupState | undefined {
    ...
    let toggleOn = false
    for (const event of session.events) {
      if (event.type === 'qunliao/setup') setup = event.data
      else if (event.type === 'qunliao/state') toggleOn = event.data.toggleOn
    }
    if (setup === undefined) return undefined
    // 兼容 v2 旧群：setup 没有 rounds 时补插件默认值，避免 undefined 写入新事件。
    const normalized: QunliaoSetupData = {
      ...setup,
      rounds: setup.rounds ?? this.config.maxRounds,
    }
    const state: QunliaoGroupState = { setup: normalized, toggleOn, running: false, abort: null }
  ...
  create(session: Session, input: QunliaoCreateInput): string {
    ...
    const members: QunliaoMember[] = input.members.map((member, index) => ({
      id: `m${index + 1}`,
  ...
  async say(session: Session, text: string): Promise<string> {
    const state = this.hydrate(session)
    if (state === undefined) throw new Error('该会话不是群聊：请先通过「新群聊」创建')
    const trimmed = text.trim()
    if (trimmed.length === 0) return '发言内容为空'
  ...
  async setToggle(session: Session, on: boolean): Promise<string> {
    const state = this.hydrate(session)
    if (state === undefined) throw new Error('该会话不是群聊：请先通过「新群聊」创建')
  ...
  addMembers(session: Session, additions: readonly {
    ...
    const state = this.hydrate(session)
    if (state === undefined) throw new Error('该会话不是群聊：请先通过「新群聊」创建')
    if (additions.length === 0) throw new Error('没有要添加的成员')
    ...
    for (let index = 0; index < additions.length; index += 1) {
      ...
      if (name.length === 0) throw new Error('成员名字不能为空')
      if (existingNames.has(name)) throw new Error(`成员名字重复：${name}`)
      existingNames.add(name)
      if (identity.length === 0) throw new Error(`成员「${name}」缺少身份设定`)
      if (provider.length === 0) throw new Error(`成员「${name}」缺少模型提供方`)
      if (model.length === 0) throw new Error(`成员「${name}」缺少模型`)
      created.push({
  ...
  setRounds(session: Session, rounds: number): string {
    ...
    if (clamped === state.setup.rounds) return `讨论轮数仍为 ${clamped} 轮`
    const setup: QunliaoSetupData = {
      name: state.setup.name,
  ...
  private clampRounds(rounds: number | undefined): number {
    if (rounds === undefined) return this.config.maxRounds
  ...
  muteMember(session: Session, target: string, muted: boolean): string {
    ...
    if (member === undefined) throw new Error(`找不到成员：${target}`)
    const members = state.setup.members.map(candidate =>
      candidate.id === member.id ? { ...candidate, muted } : candidate)
    const setup: QunliaoSetupData = {
      name: state.setup.name,
      members,
      rounds: state.setup.rounds,
  ...
  private async waitIdle(state: QunliaoGroupState): Promise<void> {
    while (state.running) {
  ...
  stateText(session: Session): string {
    const state = this.hydrate(session)
    if (state === undefined) return '该会话不是群聊'
    const lines = [
      `群名：${state.setup.name}`,
  ...
  private lastUserSeq(session: Session): number | undefined {
    for (let index = session.events.length - 1; index >= 0; index -= 1) {
      const event = session.events[index]
      if (event !== undefined && event.type === 'user/message') return event.seq
    }
    return undefined
  }
  ...
  private async runDiscussion(
    session: Session,
    state: QunliaoGroupState,
    mode: QunliaoDiscussionMode,
  ): Promise<void> {
    ...
    try {
      ...
      while (round < maxRounds && queue.length > 0 && !state.abort.signal.aborted) {
        ...
        if (spoken.has(speaker.id)) continue
        let reply: string
        try {
  ...
  private userTextBySeq(session: Session, seq: number): string {
    for (const event of session.events) {
      if (event.type === 'user/message' && event.seq === seq) return textOfContent(event.data.content)
    }
    return ''
  }

  private appendState(session: Session, state: QunliaoGroupState): void {
    const data: QunliaoStateData = { toggleOn: state.toggleOn, running: state.running, updatedAt: Date.now() }
    session.append('qunliao/state', data)
  }

  private appendMessage(
    session: Session,
    speaker: { speakerId: string; speakerName: string; emoji?: string },
    text: string,
    turn = 1,
  ): void {
    const messageId = randomUUID()
    const start: QunliaoMessageStartData = {
      messageId,
  ...
  private appendError(session: Session, text: string): void {
    const data: QunliaoErrorData = { text }
    session.append('qunliao/error', data)
  }

  /** 默认发言实现：用成员自己的 provider/model 直接调用大模型。 */
  private async defaultSpeaker(
    ctx: Context,
    session: Session,
    member: QunliaoMember,
    round: number,
    ...
    if (text.length === 0) throw new Error('模型未返回有效内容')
    return text
  }
}
...
function finishError(finish: FinishReason): Error | undefined {
  switch (finish.kind) {
    ...
    case 'max-tokens': {
      const error = new Error('发言超过输出上限（不完整）') as Error & { code?: string }
