...
import { QunliaoEngine } from "./engine.js";
export { QunliaoEngine, parseMentions, buildTranscript, validateCreateInput } from "./engine.js";
...
function parseSubcommand(rawInput) {
    ...
    if (space < 0)
        return { kind: trimmed.toLowerCase(), arg: '' };
    return { kind: trimmed.slice(0, space).toLowerCase(), arg: trimmed.slice(space + 1).trim() };
}
/** 命令处理器：把子命令转发给引擎。 */
async function handleCommand(ctx, engine, invocation) {
    ...
    try {
        switch (kind) {
            ...
            case 'add': {
                ...
                return { kind: 'success', text: engine.addMembers(session, additions) };
            }
            case 'rounds': {
            ...
            case 'say':
                if (arg.length === 0)
                    return { kind: 'error', text: '发言内容为空' };
                // 用户消息同步落库；讨论在后台进行，命令立即返回，客户端不被阻塞。
                void engine.say(session, arg).catch((error) => {
            ...
            case 'toggle':
                ...
                void engine.setToggle(session, true).catch((error) => {
