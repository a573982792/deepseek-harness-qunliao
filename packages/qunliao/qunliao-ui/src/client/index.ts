...
import type { WorkspaceRuntime } from '@deepseek-ai/dsh-client-runtime/client'
import type {} from '@deepseek-ai/dsh-api-remotes/client'
import type {} from '@deepseek-ai/dsh-client-locale/client'
import type {} from '@deepseek-ai/dsh-client-runtime/client'
import type {} from '@deepseek-ai/dsh-client-ui-conversation/client'
import type {} from '@deepseek-ai/dsh-client-ui-sidebar/client'
import { MemberMessage } from './MemberMessage.tsx'
import { GroupToggleButton } from './GroupToggleButton.tsx'
import type { GroupToggleInjected } from './GroupToggleButton.tsx'
import { GroupManageButton } from './GroupManageButton.tsx'
import type { GroupManageInjected } from './GroupManageButton.tsx'
import { GroupComposer, selectQunliaoComposer } from './GroupComposer.tsx'
import type { GroupComposerInjected } from './GroupComposer.tsx'
import { NewGroupChatEntry } from './NewGroupChat.tsx'
import type { NewGroupChatInjected } from './NewGroupChat.tsx'
import {
  qunliaoMemberDefinition, qunliaoSetupDefinition, qunliaoStateDefinition,
} from './definitions.ts'
import { en, zh, type QunliaoUiKey } from './locales.ts'
...
/** 等待会话绑定就绪（最多 5 秒）。 */
async function waitForSession(ctx: ClientContext, sessionId: SessionId, timeoutMs = 5000): Promise<boolean> {
  const deadline = Date.now() + timeoutMs
  while (Date.now() < deadline) {
    if (ctx.sessions.binding(sessionId)?.session !== undefined) return true
...
async function runQunliaoCommand(
  ctx: ClientContext,
  sessionId: SessionId,
  line: string,
  ...
  if (session === undefined) return { ok: false, text: '会话尚未就绪' }
  const result = await session.command(line)
  if (!result.ok) return { ok: false, text: `${result.error.code}: ${result.error.message}` }
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.slots.inject('conversation.composer', () => ctx.slots.register({
    ...
    priority: -20,
    locale: NS,
    inject: (sessionId): GroupComposerInjected => ({
      say: async (text) => {
  ...
  ctx.slots.inject('conversation.session.header.actions', () => ctx.slots.register({
    ...
    order: 20,
    locale: NS,
    inject: (sessionId): GroupToggleInjected => ({
      toggle: async (on) => {
  ...
  ctx.slots.inject('conversation.session.header.actions', () => ctx.slots.register({
    ...
    order: 30,
    locale: NS,
    inject: (sessionId): GroupManageInjected => {
